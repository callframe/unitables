...
- Multi-code-point writers come in pairs: `f_length(...)` returns exactly what `f(..., dst)` will write, and `f` takes no capacity, so the caller sizes `dst`. No partial writes, ever. A null `dst` makes the writer count without writing, which is how each `_length` stays a one-line wrapper over its writer and the two can never disagree.
